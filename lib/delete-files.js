...
module.exports = async function ({ source, target, dry, silent, moveTo, noCache }) {
  ...
  fs.writeFile('delete-files.txt', `
    ...
    File List:
        ${toDelete.map(file => `Source ${file.sourceFile}
            Duplicates:
  ...
  if (!dry) {
    ...
    const moveTarget = f => path.join(moveTo, f.sourceFile);
    const removalAction = moveTo
      ? f => fs.move(path.join(sourceDir.path, f.sourceFile.path), moveTarget(f))
      : f => fs.remove(path.join(sourceDir.path, f.sourceFile.path));
